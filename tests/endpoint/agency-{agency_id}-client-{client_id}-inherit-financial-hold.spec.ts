...
describe('agency-{agency_id}-client-{client_id}-inherit-financial-hold', () => {
  ...
  describe('post', () => {
    ...
    it('should respond with 202 update inherit financial hold', async () => {
      await AgencyClientsProjectionScenarios.createRecord({
        agency_id: agencyId
      });
      const res = await api
        .post(`/agency/${agencyId}/client/${clientId}/inherit-financial-hold`)
        .set(headers)
        .send(payload);
    ...
    it('should respond with 400 Validation Error', async () => {
      ...
      const res = await api.post(`/agency/${agencyId}/client/21213/inherit-financial-hold`).set(headers).send(payload);
