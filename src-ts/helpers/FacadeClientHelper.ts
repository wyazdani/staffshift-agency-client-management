...
export class FacadeClientHelper {
   ...
   */
  async getAgencySiteClientDetails(agencyId: string, organisationId: string, siteId: string, options: {[key: string]: any}) {
    if (!options) {
      options = {
   ...
   * @returns Boolean
   */
  async getAgencyDetailsFromId(agencyId: string) {
    const client = FacadeClientHelper.getInstance();
    ...
    return new Promise((resolve, reject) => {
      api.getAgency(agencyId, tokenAuthorization, options, (error: any, data: any, response: any) => {
        let item = null;
        if (error) {
          if (response) {
