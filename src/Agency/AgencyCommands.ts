...
import {AgencyCommandEnum, AgencyEventEnum} from './AgencyEnums';
import {ObjectID} from 'mongodb';
import {AgencyAggregate} from './AgencyAggregate';
import {AgencyEventInterface} from './Interfaces';
import {GenericObjectInterface} from 'GenericObjectInterface';

type AgencyCommandsType = {
  [key in AgencyCommandEnum]: (aggregate: AgencyAggregate, command: GenericObjectInterface) =>
    Promise<AgencyEventInterface[]>;
};

export const AgencyCommands: AgencyCommandsType = {
  [AgencyCommandEnum.ADD_AGENCY_CONSULTANT_ROLE]:
    async (aggregate: AgencyAggregate, command: GenericObjectInterface): Promise<AgencyEventInterface[]> => {
