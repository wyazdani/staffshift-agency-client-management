...
import {AgencyClientEventEnum} from './AgencyClientEnums';
import {AgencyClientAggregateRecordInterface, AgencyClientConsultantInterface, AgencyClientEventInterface} from './Interfaces';

type AgencyClientWriteProjectionType = {
  [key in AgencyClientEventEnum]: (aggregate: AgencyClientAggregateRecordInterface, event: AgencyClientEventInterface) =>
...
export const AgencyClientWriteProjection: AgencyClientWriteProjectionType = {
  [AgencyClientEventEnum.AGENCY_CLIENT_LINKED]:
    (aggregate: AgencyClientAggregateRecordInterface, event: AgencyClientEventInterface): AgencyClientAggregateRecordInterface => {
  ...
  [AgencyClientEventEnum.AGENCY_CLIENT_CONSULTANT_UNASSIGNED]:
    (aggregate: AgencyClientAggregateRecordInterface, event: AgencyClientEventInterface): AgencyClientAggregateRecordInterface => {
      aggregate.consultants =
        differenceWith(aggregate.consultants, [event.data], (value, other) => ((value._id == other._id)));
